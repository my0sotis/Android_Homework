...
public class PuzzleGameBoard {
     ...
     */
    public void reset() {
        PuzzleGameTile[][] Tmp = new PuzzleGameTile[mRows][mColumns];
        for (int i = 0; i < mRows; i++) {
            for (int j = 0; j < mColumns; j++) {
                boolean ismatch = false;
                for (int k = 0; k < mRows; k++) {
                    for (int l = 0; l < mColumns; l++) {
                        if(mTileGrid[k][l].getOrderIndex() == i * mColumns + j) {
                            Tmp[i][j] = mTileGrid[k][l];
                            ismatch = true;
                        }
                        if(ismatch) break;
                    }
    ...
    /**
     * Checks if two tiles neighbor each other on the game board along the vertical and horizontal
     * axes:
    ...
    public boolean areTilesNeighbors(
        ...
        if((Math.abs(firstTileCol-secondTileCol)==1&&(firstTileRow==secondTileRow))
            ||((Math.abs(firstTileRow-secondTileRow)==1&&(firstTileCol==secondTileCol))))
            return true;
        return false;
    }
    ...
    public final void swapTiles(
       ...
       // TODO - swap two tiles at the given indices
        PuzzleGameTile tmp = mTileGrid[firstTileRow][firstTileCol];
        mTileGrid[firstTileRow][firstTileCol] = mTileGrid[secondTileRow][secondTileCol];
        mTileGrid[secondTileRow][secondTileCol] = tmp;
    }
    ...
    private int countInversions(int i, int j) {
        int inversions = 0;
        int tileNum = mTileGrid[i][j].getOrderIndex();
        int k = i; int l = j;
        for (; k < mRows; k++,l=0) {
            for (; l < mColumns; l++) {
                if(tileNum > mTileGrid[k][l].getOrderIndex() && tileNum != getTotalTileCount() - 1)
                    inversions++;
            }
        }
        return inversions;
